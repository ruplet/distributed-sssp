    ...
    for solution in Path(".").iterdir():
        if solution.is_dir() and not solution.name in ["tests", "outputs"]:
            ...
            for test in Path("tests").iterdir():
                if test.name not in ['bigcycle_100100_24']:
                ...
                if local:
                    ...
                    command = "srun"
                execution = subprocess.run([command, "-n", str(workers), "./test_command.sh", solution.name, test.name], capture_output=True, timeout=TIMEOUT)
