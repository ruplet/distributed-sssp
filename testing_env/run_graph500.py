        ...
        if solution.is_dir() and not solution.name in ["tests", "outputs"]:
            ...
            for test in Path("tests").iterdir():
                ...
                if local:
                    ...
                    command = "srun"
                execution = subprocess.run([command, "-n", str(workers), "./test_command.sh", solution.name, test.name, "40", "--nolocal-bypass"], capture_output=True, timeout=TIMEOUT)
